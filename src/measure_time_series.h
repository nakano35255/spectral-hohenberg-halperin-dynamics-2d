#ifndef SHHD_MEASURE_TIME_SERIES_H
#define SHHD_MEASURE_TIME_SERIES_H
...
enum class TimeSeriesColumnKind {
    EnergyTotal,
    EnergyKinetic,
    EnergyFree,
    ...
    MomentumGradientL2,
    OrderParameterL2,
    OrderParameterGradientL2
};

struct TimeSeriesColumn {
    TimeSeriesColumnKind kind;
    int component = -1;
    std::string name;
};

class TimeSeriesMeasure : public Measure {
private:
    ...
    double compute_spectral_scalar_sum(const Complex* field, bool with_k2) const;
    double compute_spectral_vector_sum(const Complex* field_x, const Complex* field_y, bool with_k2) const;
