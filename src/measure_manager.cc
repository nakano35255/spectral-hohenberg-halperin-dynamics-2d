...
// ---------------------------------------------------------------------- //
FluxRequest MeasurementManager::flux_request() const {
    FluxRequest request;

    for (const auto& measure : measures_) {
        const FluxRequest measure_request = measure->flux_request();

        request.density = request.density || measure_request.density;
        request.order_parameter = request.order_parameter || measure_request.order_parameter;
        request.momentum = request.momentum || measure_request.momentum;
    }

    return request;
}
// ---------------------------------------------------------------------- //
void MeasurementManager::observe(const State& state, FourierTransform2D& fft, const FluxBuffer& flux, int step, double time) {
    workspace_.begin_step(step);

    for (const auto& measure : measures_) {
        measure->observe(state, fft, workspace_, flux, step, time);
    }
}
// ---------------------------------------------------------------------- //
