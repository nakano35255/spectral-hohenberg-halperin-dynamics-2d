...
namespace {
    ...
    double reference_density(const State& state, const Domain2D& domain) {
        ...
        if (rho0 <= 0.0) {
            throw std::runtime_error("set momentum equilibrium/gaussian/incompressible requires positive reference density.");
        }
...
EquilibriumGaussianIncompressibleMomentumInitialCondition::EquilibriumGaussianIncompressibleMomentumInitialCondition(
    const Params& params,
    std::shared_ptr<const MomentumInitialConditionCommandBase> command
) : MomentumInitialCondition(params, command) {
    (void)params;

    const auto cfg = std::dynamic_pointer_cast<const EquilibriumGaussianIncompressibleMomentumInitialConditionCommand>(command);
    if (!cfg) {
        throw std::runtime_error("EquilibriumGaussianIncompressibleMomentumInitialCondition: invalid command type.");
    }
...
void EquilibriumGaussianIncompressibleMomentumInitialCondition::apply(State& state, const Domain2D& domain, const SpectralMask2D& spectral_mask) const {
    Complex* momentum = nullptr;

    if (direction_ == 0) momentum = state.jx_hat_data();
    else if (direction_ == 1) momentum = state.jy_hat_data();
    else throw std::runtime_error("EquilibriumGaussianIncompressibleMomentumInitialCondition: direction must be 0 or 1.");
    ...
    for (const SpectralMode2D& mode : spectral_mask.active_modes()) {
        ...
        const double k = std::sqrt(mode.k2);
        const double component = (direction_ == 0) ? (-mode.ky / k) : (mode.kx / k);
        const Complex transverse = sigma * gaussian_mode(seed_, 0, mode, domain.ny_global());
        momentum[mode.index] = component * transverse;
    }
}
