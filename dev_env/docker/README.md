...
environment for `spectral-fluctuating-binary-fluid`.
...
```bash
cd /Users/nakano/github/spectral-fluctuating-binary-fluid
docker build -t spectral-fluctuating-binary-fluid .
```
...
docker run --rm -it \
  -v "$PWD":/work \
  -w /work \
  spectral-fluctuating-binary-fluid \
  bash
```
...
5. Run `docker build -t spectral-fluctuating-binary-fluid .`.
