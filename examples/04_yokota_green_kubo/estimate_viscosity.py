...
EXAMPLE = ROOT / "examples" / "04_yokota_green_kubo"
RESULTS = EXAMPLE / "results"

DEFAULT_YKGK = RESULTS / "yokota_green_kubo_incompressible.dat"
DEFAULT_TIME_SERIES = RESULTS / "time_series_incompressible.dat"
...
def write_summary(
    ...
    with path.open("w") as handle:
        ...
        handle.write(f"# sources {len(sources)}\n")
        ...
        handle.write(
            "# columns mode_index kx ky |k| tau_min tau_max "
            "S_last S_tail_mean S_tail_sem S_tail_std tail_points sample_count_min\n"
        )
        for summary in summaries:
            handle.write(
                ...
                f"{summary.s_tail_sem:.16e} "
                f"{summary.s_tail_std:.16e} "
                f"{summary.tail_points:d} "
                f"{summary.sample_count_min:d}\n"
            )
...
def plot_tau_curves(
    ...
    for mode_index in selected_modes:
        ...
        label = f"m={mode_index}, |k|={math.hypot(points[-1].kx, points[-1].ky):.3g}"
...
def plot_k_spectrum(
    ...
    plt = import_pyplot()
    q_values = np.asarray([summary.k_abs for summary in summaries], dtype=float)
    ...
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel(r"$q=|\boldsymbol{k}|$")
...
def main() -> None:
    ...
    if not args.no_plots:
        ...
        plot_k_spectrum(
            output_dir / "yokota_green_kubo_spectrum.png",
            summaries,
            eta,
            config,
            args.a_uv,
