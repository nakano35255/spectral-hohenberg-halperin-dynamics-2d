...
def main_data_root(sc_label):
    return MAIN_ROOT / sc_label


def cache_path(sc_label):
    return CACHE_DIR / f"main_N128_auv32_{sc_label.lower()}_steady_last_half.npz"
...
def load_rows(sc_label):
    path = cache_path(sc_label)
    signature = source_signature(sc_label)
    cached = load_cache(signature, path)
    if cached is not None:
        print(f"loaded cache {path}")
        return cached

    data_root = main_data_root(sc_label)
    rows = [
        summarize_case(d0_dir)
        for d0_dir in sorted(data_root.glob("D0_*"), key=lambda path: numeric_label(path, "D0")[1])
    ]
    if not rows:
        raise FileNotFoundError(f"no D0 directories in {data_root}")
    save_cache(rows, signature, path)
    return rows


def source_signature(sc_label):
    entries = []
    data_root = main_data_root(sc_label)
    for d0_dir in sorted(data_root.glob("D0_*"), key=lambda path: numeric_label(path, "D0")[1]):
    ...
    return {"version": 3, "main_root": str(data_root), "entries": entries}
...
def save_cache(rows, signature, path):
