...
This directory keeps the standalone incompressible nonlinear Green-Kubo data,
including the older `eta0=0.1` and `eta0=0.5` runs:
...
Yokota MD comparison data lives separately in
`examples/03_yokota_green_kubo/yokota_data/`. That directory contains the
digitized MD data, the `eta0=0.2/0.25/0.26/0.27` comparison runs, and their
comparison figures.

Raw run directories under `raw_data/` should keep the generated
`runs/` and `results/` subdirectories. Per-sample seeds are recorded in
`runs/input_*.script`, so separate seed manifest files are intentionally
omitted. For example:

```text
raw_data/eta0_0p1_grid256_L256_dt0p01_T25000_diag_n1152/
  runs/
  results/
```
...
python3 examples/03_yokota_green_kubo/incompressible_nonlinear/estimate_viscosity.py \
  --input-glob "examples/03_yokota_green_kubo/incompressible_nonlinear/raw_data/eta0_0p1_grid256_L256_dt0p01_T25000_diag_n1152/results/yokota_green_kubo_*.dat" \
  --input-script "examples/03_yokota_green_kubo/incompressible_nonlinear/raw_data/eta0_0p1_grid256_L256_dt0p01_T25000_diag_n1152/runs/input_000.script" \
  --output-dir "examples/03_yokota_green_kubo/incompressible_nonlinear/processed_data/eta0_0p1_grid256_L256_dt0p01_T25000_diag_n1152" \
  --figure-dir "examples/03_yokota_green_kubo/incompressible_nonlinear/figures/eta0_0p1_grid256_L256_dt0p01_T25000_diag_n1152"
```

The processed output contains:

```text
processed_data/eta0_0p1_grid256_L256_dt0p01_T25000_diag_n1152/
  metadata.csv
